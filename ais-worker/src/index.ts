...
class AISWorker {
  ...
  private batchBuffer: VesselPosition[] = [];
  ...
  private async persistToSupabase(batch: VesselPosition[]): Promise<void> {
  ...
  private generateMockVessels(): VesselPosition[] {
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      this.state.reconnectDelay = Math.min(
        ...
        AIS_CONFIG.maxReconnectDelayMs,
      );
  ...
  private startStatusReporter(): void {
    setInterval(() => {
      ...
      console.log(
        `[AIS Worker] Status: ${this.state.connected ? 'LIVE' : 'DISCONNECTED'} | ` +
