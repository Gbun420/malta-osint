...
const VESSEL_TYPES = ['cargo', 'passenger', 'tanker', 'fishing', 'tug'];
...
let mockVessels = generateVessels(30);
...
export function createMockAISServer(port = 7701) {
    const server = http.createServer((req, res) => {
        ...
        if (url.pathname === '/vessels') {
            ...
            if (bbox) {
                ...
                filtered = mockVessels.filter((v) => v.lat >= minLat && v.lat <= maxLat && v.lon >= minLon && v.lon <= maxLon);
            }
