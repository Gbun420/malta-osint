...
/**
 * THIRD EYE — Maritime Intelligence
...
function connectAisStream() {
  ...
  // Map AIS ship types to THIRD EYE categories
  const getThirdEyeShipType = (typeCode: number) => {
  ...
  ws.on("message", (data) => {
    try {
      ...
      if (parsed.MessageType === "PositionReport" && parsed.Message?.PositionReport) {
        ...
        existing.type = getThirdEyeShipType(staticData.Type);
      }
...
async function fetchVesselApiFallback() {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[VesselAPI] HTTP ${res.status}`);
      return;
    }
