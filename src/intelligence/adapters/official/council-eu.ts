...
const FEEDS = [
  { name: 'European Commission Press Releases', url: 'https://ec.europa.eu/commission/presscorner/api/rss?language=en' },
];

export async function fetchCouncilEU(): Promise<AdapterResult<IntelligenceEvent>> {
  const sourceId = 'council-eu-rss';
  ...
  for (const feed of FEEDS) {
    try {
      ...
      for (const item of items) {
        ...
        events.push({
          id,
          canonicalKey: `eu-pc-${pubDate}-${title.slice(0, 40)}`,
          title,
